...
if (params.DummyMode)
    ...
    if(params.use_real_dummy == 1)
        ...
        EOG = 15 * pulstran(t, c+0.5, 'rectpuls', ExtendFactor*1/5) .* (rand>0.5+sigmoid) ...
               + 0.5 .* randn(params.BufferLength_Biosemi,2) ...
               + 10;
        ...
        if (length(buffer.X_train) == 1)
            ...
            if buffer.Calib_or_Acquisition(1) == 1
                y_sig = linspace(buffer.Y_train(1)-params.default_fixation_y, ...
                    buffer.Y_train(1)-params.default_fixation_y, ...
                    params.BufferLength_Biosemi)';
                
                EOG(:, 1) = EOG(:, 1) + linspace(buffer.X_train(1), ...
                    buffer.X_train(1), params.BufferLength_Biosemi)';

                EOG(:, 2) = EOG(:, 2) + y_sig ...
                    + 0.2 * linspace(buffer.X_train(1), ...
                    buffer.X_train(1), params.BufferLength_Biosemi)';
                
            elseif buffer.Calib_or_Acquisition(1) == 0
                y_sig = linspace(buffer.Y_train(1)-params.default_fixation_y, ...
                    buffer.Y_train(2)-params.default_fixation_y, ...
                    params.BufferLength_Biosemi)';
...
for i=1:n_data_bk
    ...
    [range, t, nDeletedPrevRange] = eogdetection_msdw_online(b.dataqueue,...
        ...
        b.histogram, p.nBin4Histogram,p.alpha, p.v, p.threshold_balance);

...
function [ dist ,table, match_pair] = fastDPW( data_ref, data_test, max_slope_length, speedup_mode, window_width)
    %array 길이 계산
    ...
    for j=2:size_test
        ...
        for i=r_start:r_end
            ...
            table(i, j) = min_dist + abs(v_ref(min_id_ref,i)-v_test(min_id_test,j)); %root;
        end
    end
